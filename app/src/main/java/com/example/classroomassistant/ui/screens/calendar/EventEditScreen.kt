...
@Composable
fun EventEditScreen(vm: CalendarViewModel, onDone: () -> Unit) {
    val title = remember { mutableStateOf("") }
    val date = remember { mutableStateOf(java.time.LocalDate.now().toString()) }
    val time = remember { mutableStateOf("17:00") }
    val content = remember { mutableStateOf("") }
    val type = remember { mutableStateOf("教学") }
    val important = remember { mutableStateOf(false) }
    ...
    Column(Modifier.fillMaxSize().padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
        ...
        PrimaryButton("保存") {
            vm.save(CalendarEvent(title = title.value, date = date.value, time = time.value, content = content.value, type = type.value, important = important.value), onDone)
        }
    }
}
